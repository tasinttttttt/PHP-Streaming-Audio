...
class getID3_cached_sqlite3 extends getID3
{
    ...
    * @return type
    */
    public function __construct(private $table='getid3_cache', $hide=false)
    {
        // Set table
    ...
    */
    ...
    {
        return match ($name) {
            'version_check' => "SELECT val FROM $this->table WHERE filename = :filename AND filesize = '-1' AND filetime = '-1' AND analyzetime = '-1'",
            'delete_cache' => "DELETE FROM $this->table",
            'set_version' => "INSERT INTO $this->table (filename, dirname, filesize, filetime, analyzetime, val) VALUES (:filename, :dirname, -1, -1, -1, :val)",
            'get_id3_data' => "SELECT val FROM $this->table WHERE filename = :filename AND filesize = :filesize AND filetime = :filetime",
            'cache_file' => "INSERT INTO $this->table (filename, dirname, filesize, filetime, analyzetime, val) VALUES (:filename, :dirname, :filesize, :filetime, :atime, :val)",
            'make_table' => "CREATE TABLE IF NOT EXISTS $this->table (filename VARCHAR(255) DEFAULT '', dirname VARCHAR(255) DEFAULT '', filesize INT(11) DEFAULT '0', filetime INT(11) DEFAULT '0', analyzetime INT(11) DEFAULT '0', val text, PRIMARY KEY (filename, filesize, filetime))",
            'get_cached_dir' => "SELECT val FROM $this->table WHERE dirname = :dirname",
            default => null,
        };
    }
}
