...
class getid3_matroska extends getid3_handler
{
    ...
    private int $EBMLbuffer_offset = 0;
    private int $EBMLbuffer_length = 0;
    private int $current_offset    = 0;
    private array $unuseful_elements = array(EBML_ID_CRC32, EBML_ID_VOID);
    ...
    public function Analyze()
    {
        ...
        if (isset($info['matroska']['info']) && is_array($info['matroska']['info'])) {
            foreach ($info['matroska']['info'] as $key => $infoarray) {
                if (isset($infoarray['Duration'])) {
                    ...
                    $info['playtime_seconds'] = $infoarray['Duration'] * (($infoarray['TimecodeScale'] ?? 1_000_000) / 1_000_000_000);
                    break;
                }
            }
        }
                ...
                switch ($trackarray['TrackType']) {
                    ...
                    case 1: // Video
                        ...
                        if (isset($trackarray['DefaultDuration'])) {
                            $track_info['frame_rate']  = round(1_000_000_000 / $trackarray['DefaultDuration'], 3);
                        }
                    ...
                    case 2: // Audio
                        ...
                        switch ($trackarray['CodecID']) {
                            ...
                            case 'A_FLAC':
                                ...
                                $getid3_audio = new $class($getid3_temp, self::class);
                                if ($track_info['dataformat'] == 'flac') {
        ...
        if (isset($info['matroska']['attachments']) && $this->getid3->option_save_attachments !== getID3::ATTACHMENTS_NONE) {
            foreach ($info['matroska']['attachments'] as $i => $entry) {
                if (str_starts_with($entry['FileMimeType'], 'image/') && !empty($entry['FileData'])) {
                        ...
                        switch ($element_data['id']) {
                            ...
                            case EBML_ID_CLUSTER: // The lower level element containing the (monolithic) Block structure.
                                ...
                                if (!self::$parse_whole_file) {
                                    if (isset($info['matroska']['info']) && is_array($info['matroska']['info'])) {
                                        if (isset($info['matroska']['tracks']['tracks']) && is_array($info['matroska']['tracks']['tracks'])) {
                                            if ((is_countable($info['matroska']['track_data_offsets']) ? count($info['matroska']['track_data_offsets']) : 0) == count($info['matroska']['tracks']['tracks'])) {
                                                return;
                                            }
                                        }
                                    }
    ...
    private function EnsureBufferHasEnoughData($min_data=1024)
    {
        if (($this->current_offset - $this->EBMLbuffer_offset) >= ($this->EBMLbuffer_length - $min_data)) {
            ...
            try {
                ...
                $this->EBMLbuffer_length = strlen((string) $this->EBMLbuffer);
    ...
    private static function EBMLdate2unix($EBMLdatestamp)
    {
        ...
        return round(($EBMLdatestamp / 1_000_000_000) + 978_307_200);
    }
