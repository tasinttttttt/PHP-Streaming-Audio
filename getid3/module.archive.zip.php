...
class getid3_zip extends getid3_handler
{
    public function Analyze()
    {
        ...
        if (!getid3_lib::intValueSupported($info['filesize'])) {
            $info['error'][] = 'File is larger than '.round(PHP_INT_MAX / 1_073_741_824).'GB, not supported by PHP';
            return false;
        } else {
            $EOCDsearchData    = '';
            $EOCDsearchCounter = 0;
    ...
    public function ZIPparseLocalFileHeader()
    {
        ...
        $LocalFileHeader['host_os']                   = static::ZIPversionOSLookup(($LocalFileHeader['raw']['extract_version'] & 0xFF00) >> 8);
        $LocalFileHeader['compression_method']        = static::ZIPcompressionMethodLookup($LocalFileHeader['raw']['compression_method']);
        ...
        $LocalFileHeader['flags']                     = static::ZIPparseGeneralPurposeFlags($LocalFileHeader['raw']['general_flags'], $LocalFileHeader['raw']['compression_method']);
        $LocalFileHeader['last_modified_timestamp']   = static::DOStime2UNIXtime($LocalFileHeader['raw']['last_mod_file_date'], $LocalFileHeader['raw']['last_mod_file_time']);
    ...
    public function ZIPparseCentralDirectory()
    {
        ...
        $CentralDirectory['host_os']                   = static::ZIPversionOSLookup(($CentralDirectory['raw']['extract_version'] & 0xFF00) >> 8);
        $CentralDirectory['compression_method']        = static::ZIPcompressionMethodLookup($CentralDirectory['raw']['compression_method']);
        ...
        $CentralDirectory['flags']                     = static::ZIPparseGeneralPurposeFlags($CentralDirectory['raw']['general_flags'], $CentralDirectory['raw']['compression_method']);
        $CentralDirectory['last_modified_timestamp']   = static::DOStime2UNIXtime($CentralDirectory['raw']['last_mod_file_date'], $CentralDirectory['raw']['last_mod_file_time']);
    ...
    public static function ZIPversionOSLookup($index)
    {
        ...
        return ($ZIPversionOSLookup[$index] ?? '[unknown]');
    }
    ...
    public static function ZIPcompressionMethodLookup($index)
    {
        ...
        return ($ZIPcompressionMethodLookup[$index] ?? '[unknown]');
    }
