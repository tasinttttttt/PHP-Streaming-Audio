    ...
    public function Analyze()
    {
        ...
        while ($offset < $info['avdataend']) {
            if (!getid3_lib::intValueSupported($offset)) {
                $info['error'][] = 'Unable to parse atom at offset '.$offset.' because beyond '.round(PHP_INT_MAX / 1_073_741_824).'GB limit of PHP filesystem functions';
                break;
            }
            $this->fseek($offset);
        ...
        if (!empty($info['quicktime']['comments']['chapters']) && is_array($info['quicktime']['comments']['chapters']) && (count($info['quicktime']['comments']['chapters']) > 0)) {
            ...
            for ($i = 0; $i < (is_countable($info['quicktime']['comments']['chapters']) ? count($info['quicktime']['comments']['chapters']) : 0); $i++) {
                $bookmark = array();
    ...
    public function QuicktimeParseAtom($atomname, $atomsize, $atom_data, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms)
    {
        ...
        switch ($atomname) {
            ...
            case 'ilst': // Item LiST container atom
                if ($atom_structure['subatoms'] = $this->QuicktimeParseContainerAtom($atom_data, $baseoffset + 8, $atomHierarchy, $ParseAllPossibleAtoms)) {
                    ...
                    foreach ($atom_structure['subatoms'] as $subatomarray) {
                        if (!is_integer($subatomarray['name']) || ((is_countable($subatomarray['subatoms']) ? count($subatomarray['subatoms']) : 0) != 1)) {
            ...
            case 'tvsn': // TV SeasoN
                if ($atom_parent == 'udta') {
                    ...
                    if (substr($atom_data, 2, 2) == "\x10\xB5") {
                        ...
                        while ($atomoffset < strlen((string) $atom_data)) {
                            ...
                            if ($boxsmallsize <= 1) {
                                ...
                                $atomoffset = strlen((string) $atom_data);
                                break;
                            }
                            switch ($boxsmalltype) {
                                case "\x10\xB5":
            ...
            case 'rdrf': // Reference movie Data ReFerence atom
                ...
                $atom_structure['url'] = match ($atom_structure['reference_type_name']) {
                    'url ' => $this->NoNullString(substr($atom_data, 12)),
                    'alis' => substr($atom_data, 12),
                    'rsrc' => substr($atom_data, 12),
                    default => substr($atom_data, 12),
                };
                break;
...
if (!empty($atom_structure['sample_description_table'][$i]['width']) && !empty($atom_structure['sample_description_table'][$i]['height'])) {
    ...
    $info['quicktime']['video']['resolution_y'] = $info['video']['resolution_y'];
                                ...
                                default:
                                    ...
                                    $info['audio']['lossless'] = match ($atom_structure['sample_description_table'][$i]['data_format']) {
                                        'raw ', 'alac' => true,
                                        default => false,
                                    };
                                    break;
            ...
            case 'stts': // Sample Table Time-to-Sample atom
                ...
                if ($max_stts_entries_to_scan < $atom_structure['number_entries']) {
                    $info['warning'][] = 'QuickTime atom "stts" has '.$atom_structure['number_entries'].' but only scanning the first '.$max_stts_entries_to_scan.' entries due to limited PHP memory available ('.floor($atom_structure['number_entries'] / 1_048_576).'MB).';
                }
            ...
            case 'ssrc': // non-primary SouRCe atom
                for ($i = 0; $i < strlen((string) $atom_data); $i += 4) {
...
/* due to lack of known documentation, this is a kludge implementation. If you know of documentation on how mdat is properly structed, please send it to info@getid3.org */
            ...
            case "\xA9".'xyz':  // GPS latitude+longitude+altitude
                $atom_structure['data'] = $atom_data;
                if (preg_match('#([\\+\\-][0-9\\.]+)([\\+\\-][0-9\\.]+)([\\+\\-][0-9\\.]+)?/$#i', (string) $atom_data, $matches)) {
            ...
            case 'NCVW': // Nikon Camera preVieW image
                // http://www.sno.phy.queensu.ca/~phil/exiftool/TagNames/Nikon.html
                if (preg_match('/^\xFF\xD8\xFF/', (string) $atom_data)) {
                    $atom_structure['data'] = $atom_data;
                    $atom_structure['image_mime'] = 'image/jpeg';
    ...
    public function QuicktimeParseContainerAtom($atom_data, $baseoffset, &$atomHierarchy, $ParseAllPossibleAtoms)
    {
        ...
        $subatomoffset  = 0;
        $subatomcounter = 0;
        if ((strlen((string) $atom_data) == 4) && (getid3_lib::BigEndian2Int($atom_data) == 0x00000000)) {
            return false;
        }
        while ($subatomoffset < strlen((string) $atom_data)) {
    ...
    public function QuicktimeLanguageLookup($languageid)
    {
        ...
        return ($QuicktimeLanguageLookup[$languageid] ?? 'invalid');
    }
    ...
    public function QuicktimeVideoCodecLookup($codecid)
    {
        ...
        return ($QuicktimeVideoCodecLookup[$codecid] ?? '');
    }
    ...
    public function QuicktimeAudioCodecLookup($codecid)
    {
        ...
        return ($QuicktimeAudioCodecLookup[$codecid] ?? '');
    }
    ...
    public function QuicktimeDCOMLookup($compressionid)
    {
        ...
        return ($QuicktimeDCOMLookup[$compressionid] ?? '');
    }
    ...
    public function QuicktimeColorNameLookup($colordepthid)
    {
        ...
        return ($QuicktimeColorNameLookup[$colordepthid] ?? 'invalid');
    }
    ...
    public function QuicktimeSTIKLookup($stik)
    {
        ...
        return ($QuicktimeSTIKLookup[$stik] ?? 'invalid');
    }
    ...
    public function QuicktimeIODSaudioProfileName($audio_profile_id)
    {
        ...
        return ($QuicktimeIODSaudioProfileNameLookup[$audio_profile_id] ?? 'ISO Reserved / User Private');
    }
    ...
    public function QuicktimeIODSvideoProfileName($video_profile_id)
    {
        ...
        return ($QuicktimeIODSvideoProfileNameLookup[$video_profile_id] ?? 'ISO Reserved Profile');
    }
    ...
    public function QuicktimeContentRatingLookup($rtng)
    {
        ...
        return ($QuicktimeContentRatingLookup[$rtng] ?? 'invalid');
    }
    ...
    public function QuicktimeStoreAccountTypeLookup($akid)
    {
        ...
        return ($QuicktimeStoreAccountTypeLookup[$akid] ?? 'invalid');
    }
    ...
    public function QuicktimeStoreFrontCodeLookup($sfid)
    {
        ...
        return ($QuicktimeStoreFrontCodeLookup[$sfid] ?? 'invalid');
    }
    ...
    public function QuicktimeParseNikonNCTG($atom_data)
    {
        ...
        $offset = 0;
        $datalength = strlen((string) $atom_data);
        $parsed = array();
...
echo 'QuicktimeParseNikonNCTG()::unknown $data_size_type: '.$data_size_type.'<br>';
            ...
            switch ($record_type) {
                ...
                case 0x02000024: // WorldTime
                    ...
                    $datedisplayformat = match (getid3_lib::BigEndian2Int(substr($data, 3, 1))) {
                        2 => 'D/M/Y',
                        1 => 'M/D/Y',
                        default => 'Y/M/D',
                    };
            ...
            $tag_name = ($NCTGtagName[$record_type] ?? '0x'.str_pad(dechex($record_type), 8, '0', STR_PAD_LEFT));
    ...
    public function CopyToAppropriateCommentsSection($keyname, $data, $boxname='')
    {
        ...
        if ($boxname && ($boxname != $keyname)) {
            $comment_key = ($handyatomtranslatorarray[$boxname] ?? $boxname);
        ...
        if ($comment_key) {
            if ($comment_key == 'picture') {
                if (!is_array($data)) {
                    $image_mime = '';
                    if (preg_match('#^\x89\x50\x4E\x47\x0D\x0A\x1A\x0A#', (string) $data)) {
                        $image_mime = 'image/png';
                    } elseif (preg_match('#^\xFF\xD8\xFF#', (string) $data)) {
                        $image_mime = 'image/jpeg';
                    } elseif (preg_match('#^GIF#', (string) $data)) {
                        $image_mime = 'image/gif';
                    } elseif (preg_match('#^BM#', (string) $data)) {
    ...
    public function NoNullString($nullterminatedstring)
    {
        ...
        if (substr($nullterminatedstring, strlen((string) $nullterminatedstring) - 1, 1) === "\x00") {
            return substr($nullterminatedstring, 0, strlen((string) $nullterminatedstring) - 1);
        }
    ...
    */
    ...
    public function quicktime_time_to_sample_table($info)
    {
        ...
        foreach ($res as $value) {
            ...
            if ((is_countable($stbl_res) ? count($stbl_res) : 0) > 0) {
                $stts_res = array();
                $this->search_tag_by_key($value[1], 'time_to_sample_table', $value[0], $stts_res);
                if ((is_countable($stts_res) ? count($stts_res) : 0) > 0) {
        ...
        foreach ($res as $value) {
            ...
            if ((is_countable($stbl_res) ? count($stbl_res) : 0) > 0) {
                ...
                foreach ($ts_res as $value) {
                    ...
                    if ((str_starts_with($stbl_res[0][0], $prefix)) && ($ts_prefix_len < strlen($prefix))) {
