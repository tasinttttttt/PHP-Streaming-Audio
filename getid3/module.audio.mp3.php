...
class getid3_mp3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (!empty($CurrentDataLAMEversionString) && (substr($CurrentDataLAMEversionString, 0, 6) == 'LAME3.') && !preg_match('[0-9\)]', substr($CurrentDataLAMEversionString, -1))) {
            ...
            $CurrentDataLAMEversionString = match (substr($CurrentDataLAMEversionString, -1)) {
                'a', 'b' => substr($CurrentDataLAMEversionString, 0, -1),
            };
            if (($PossiblyLongerLAMEversion_String = strstr($PossiblyLongerLAMEversion_Data, (string) $CurrentDataLAMEversionString)) !== false) {
                if (substr($PossiblyLongerLAMEversion_String, 0, strlen((string) $CurrentDataLAMEversionString)) == $CurrentDataLAMEversionString) {
                    ...
                    if (empty($info['audio']['encoder']) || (strlen($PossiblyLongerLAMEversion_NewString) > strlen((string) $info['audio']['encoder']))) {
        ...
        $info['audio']['dataformat'] = match ($info['mpeg']['audio']['layer'] ?? '') {
            1, 2 => 'mp'.$info['mpeg']['audio']['layer'],
            3 => ''
        };
    ...
    public function GuessEncoderOptions()
    {
        ...
        if (!empty($thisfile_mpeg_audio_lame['lowpass_frequency'])) {
            $ExplodedOptions = explode(' ', (string) $encoder_options, 4);
                ...
                if (substr($headerstring, $VBRidOffset + 120, 4) == 'LAME') {
                    ...
                    if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.90') {
                        ...
                        if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.94b') {
                            ...
                            $thisfile_mpeg_audio_lame_RGAD['peak_amplitude'] = (float) ((getid3_lib::BigEndian2Int(substr($headerstring, $LAMEtagOffsetContant + 0xA7, 4))) / 8_388_608);
                        } else {
                        ...
                        $thisfile_mpeg_audio_lame['mp3_gain_factor'] = 2 ** ($thisfile_mpeg_audio_lame['mp3_gain_db'] / 6);
    ...
    public function getOnlyMPEGaudioInfo($avdataoffset, $BitrateHistogram=false)
    {
        ...
        $sync_seek_buffer_size = strlen((string) $header);
        $SynchSeekOffset = 0;
        while ($SynchSeekOffset < $sync_seek_buffer_size) {
            ...
            if (($SynchSeekOffset + 1) >= strlen((string) $header)) {
                    ...
                    if ($BitrateHistogram) {
                        ...
                        for ($current_segment = 0; $current_segment < $max_scan_segments; $current_segment++) {
                            ...
                            if ($current_segment > 0) {
                                ...
                                for ($j = 0; $j < (strlen((string) $buffer_4k) - 4); $j++) {
                        ...
                        if ($pct_data_scanned > 0) {
                            ...
                            foreach ($info['mpeg']['audio'] as $key1 => $value1) {
                                if (!preg_match('#_distribution$#i', (string) $key1)) {
                                    continue;
                                }
                                foreach ($value1 as $key2 => $value2) {
    ...
    public static function MPEGaudioHeaderDecode($Header4Bytes)
    {
        ...
        if (strlen((string) $Header4Bytes) != 4) {
            return false;
        }
    ...
    public static function ClosestStandardMP3Bitrate($bit_rate)
    {
        ...
        if (!isset($bit_rate_table[$round_bit_rate])) {
            if ($round_bit_rate > max($standard_bit_rates)) {
                $bit_rate_table[$round_bit_rate] = round($bit_rate, 2 - strlen((string) $bit_rate));
            } else {
    ...
    public static function LAMEpresetUsedLookup($LAMEtag)
    {
        ...
        for ($i = 8; $i <= 320; $i++) {
            $LAMEpresetUsedLookup[$i] = match ($LAMEtag['vbr_method']) {
                'cbr' => '--alt-preset '.$LAMEtag['vbr_method'].' '.$i,
                default => '--alt-preset '.$i,
            };
        }
