...
class getid3_id3v2 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (substr($header, 0, 3) == 'ID3'  &&  strlen((string) $header) == 10) {
        ...
        if ($sizeofframes > 0) {
            ...
            while (isset($framedata) && (strlen((string) $framedata) > 0)) { // cycle through until no more frame data is left to parse
                if (strlen((string) $framedata) <= static::ID3v2HeaderLength($id3v2_majorversion)) {
                ...
                if ($id3v2_majorversion == 2) {
                    ...
                    if ($frame_size < (strlen($framedata) + 4)) {
                        ...
                        if (static::IsValidID3v2FrameName($nextFrameID, $id3v2_majorversion)) {
                            // next frame is OK
                        } elseif (($frame_name == "\x00".'MP3') || ($frame_name == "\x00\x00".'MP') || ($frame_name == ' MP3') || ($frame_name == 'MP3e')) {
                            // MP3ext known broken frames - "ok" for the purposes of this test
                        } elseif (($id3v2_majorversion == 4) && (static::IsValidID3v2FrameName(substr($framedata, getid3_lib::BigEndian2Int(substr($frame_header, 4, 4), 0), 4), 3))) {
                ...
                if (($frame_size <= strlen((string) $framedata)) && (static::IsValidID3v2FrameName($frame_name, $id3v2_majorversion))) {
                    unset($parsedFrame);
                    ...
                    if ($frame_size <= strlen((string) $framedata)) {
                        if (static::IsValidID3v2FrameName(substr($framedata, $frame_size, 4), $id3v2_majorversion)) {
                    ...
                    if (!static::IsValidID3v2FrameName($frame_name, $id3v2_majorversion)) {
                    ...
                    } elseif (!isset($framedata) || ($frame_size > strlen((string) $framedata))) {
                        $info['error'][] = 'error parsing "'.$frame_name.'" ('.$framedataoffset.' bytes into the ID3v2.'.$id3v2_majorversion.' tag). (ERROR: $frame_size ('.$frame_size.') > strlen($framedata) ('.(isset($framedata) ? strlen((string) $framedata) : 'null').')).';
                    } else {
                        $info['error'][] = 'error parsing "'.$frame_name.'" ('.$framedataoffset.' bytes into the ID3v2.'.$id3v2_majorversion.' tag).';
                    }
                }
                $framedataoffset += ($frame_size + static::ID3v2HeaderLength($id3v2_majorversion));
            }
        }
    ...
    public function ParseID3v2Frame(&$parsedFrame)
    {
        ...
        $deviationbitstream = null;
        $dir = null;
        // shortcuts
        $info = &$this->getid3->info;
        $id3v2_majorversion = $info['id3v2']['majorversion'];

        $parsedFrame['framenamelong']  = static::FrameNameLongLookup($parsedFrame['frame_name']);
        if (empty($parsedFrame['framenamelong'])) {
            unset($parsedFrame['framenamelong']);
        }
        $parsedFrame['framenameshort'] = static::FrameNameShortLookup($parsedFrame['frame_name']);
        ...
        if (isset($parsedFrame['datalength']) && ($parsedFrame['datalength'] == 0)) {
            ...
            $frame_textencoding_terminator = static::TextEncodingTerminatorLookup($frame_textencoding);
            ...
            $frame_terminatorpos = strpos($parsedFrame['data'], (string) $frame_textencoding_terminator, $frame_offset);
            if (ord(substr($parsedFrame['data'], $frame_terminatorpos + strlen((string) $frame_textencoding_terminator), 1)) === 0) {
            ...
            $parsedFrame['encoding']    = static::TextEncodingNameLookup($frame_textencoding);

            $parsedFrame['description'] = trim(getid3_lib::iconv_fallback($parsedFrame['encoding'], $info['id3v2']['encoding'], $frame_description));
            $parsedFrame['data'] = substr($parsedFrame['data'], $frame_terminatorpos + strlen((string) $frame_textencoding_terminator));
            if (!empty($parsedFrame['framenameshort']) && !empty($parsedFrame['data'])) {
                $commentkey = ($parsedFrame['description'] ?: (isset($info['id3v2']['comments'][$parsedFrame['framenameshort']]) ? count($info['id3v2']['comments'][$parsedFrame['framenameshort']]) : 0));
            ...
            $frame_terminatorpos = strpos($parsedFrame['data'], (string) $frame_textencoding_terminator);
            if (ord(substr($parsedFrame['data'], $frame_terminatorpos + strlen((string) $frame_textencoding_terminator), 1)) === 0) {
                $frame_terminatorpos++; // strpos() fooled because 2nd byte of Unicode chars are often 0x00
            }
            ...
            $parsedFrame['encodingid'] = $frame_textencoding;
            $parsedFrame['encoding']   = static::TextEncodingNameLookup($parsedFrame['encodingid']);
            ...
            $IPLS_parts = array();
            if (str_contains($parsedFrame['data_raw'], "\x00")) {
            ...
            while ($frame_offset < strlen($parsedFrame['data'])) {
                ...
                $parsedFrame['type']      = static::ETCOEventLookup($parsedFrame['typeid']);
            ...
            $parsedFrame['languagename'] = static::LanguageLookup($frame_language, false);
            $parsedFrame['description']  = $frame_description;
            if (!empty($parsedFrame['framenameshort']) && !empty($parsedFrame['data'])) {
            ...
            $parsedFrame['contenttype']     = static::SYTLContentTypeLookup($parsedFrame['contenttypeid']);
            $parsedFrame['encodingid']      = $frame_textencoding;
            $parsedFrame['encoding']        = static::TextEncodingNameLookup($frame_textencoding);
            ...
            while (strlen($frame_remainingdata)) {
                $frame_offset = 0;
                $frame_terminatorpos = strpos($frame_remainingdata, (string) $frame_textencoding_terminator);
                if ($frame_terminatorpos === false) {
                    $frame_remainingdata = '';
                } else {
                    if (ord(substr($frame_remainingdata, $frame_terminatorpos + strlen((string) $frame_textencoding_terminator), 1)) === 0) {
                        $frame_terminatorpos++; // strpos() fooled because 2nd byte of Unicode chars are often 0x00
                    }
                    $parsedFrame['lyrics'][$timestampindex]['data'] = substr($frame_remainingdata, $frame_offset, $frame_terminatorpos - $frame_offset);

                    $frame_remainingdata = substr($frame_remainingdata, $frame_terminatorpos + strlen((string) $frame_textencoding_terminator));
            ...
            if (strlen($parsedFrame['data']) < 5) {
                ...
                $frame_text = (string) substr($parsedFrame['data'], $frame_terminatorpos + strlen((string) $frame_textencoding_terminator));
                ...
                if (!empty($parsedFrame['framenameshort']) && !empty($parsedFrame['data'])) {
                    $commentkey = ($parsedFrame['description'] ?: (!empty($info['id3v2']['comments'][$parsedFrame['framenameshort']]) ? count($info['id3v2']['comments'][$parsedFrame['framenameshort']]) : 0));
                    if (!isset($info['id3v2']['comments'][$parsedFrame['framenameshort']]) || !array_key_exists($commentkey, $info['id3v2']['comments'][$parsedFrame['framenameshort']])) {
            ...
            while (strlen($frame_remainingdata) >= 5) {
                ...
                $parsedFrame[$RVA2channelcounter]['channeltype']    = static::RVA2ChannelTypeLookup($frame_channeltypeid);
            ...
            if ($frame_offset >= $parsedFrame['datalength']) {
                ...
                $parsedFrame['picturetype']      = static::APICPictureTypeLookup($frame_picturetype);
                $parsedFrame['description']      = $frame_description;
                $parsedFrame['data']             = substr($parsedFrame['data'], $frame_terminatorpos + strlen((string) $frame_textencoding_terminator));
...
*/
            ...
            $frame_offset = $frame_terminatorpos + strlen((string) $frame_textencoding_terminator);
            ...
            if (!static::IsValidDateStampString($parsedFrame['purchasedate'])) {
            ...
            $parsedFrame['receivedas']        = static::COMRReceivedAsLookup($frame_receivedasid);
            ...
            if ($frame_offset < strlen($parsedFrame['data'])) {
                ...
                while ($frame_offset < strlen($parsedFrame['data'])) {
                    ...
                    $subframe['encoding']   = static::TextEncodingNameLookup($subframe['encodingid']);
