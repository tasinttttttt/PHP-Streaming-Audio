                ...
                while ($nextRIFFoffset < min($info['filesize'], $info['avdataend'])) {
                    try {
                        ...
                        if ($e->getCode() == 10) {
                            ...
                            $this->error('AVI extends beyond '.round(PHP_INT_MAX / 1_073_741_824).'GB and PHP filesystem functions cannot read that far, playtime may be wrong');
                ...
                if (isset($thisfile_riff_WAVE['iXML'][0]['data'])) {
                    ...
                    if ($parsedXML = getid3_lib::XML2array($thisfile_riff_WAVE['iXML'][0]['data'])) {
                        ...
                        if (isset($parsedXML['SPEED']['MASTER_SPEED'])) {
                            @[$numerator, $denominator] = explode('/', (string) $parsedXML['SPEED']['MASTER_SPEED']);
                            $thisfile_riff_WAVE['iXML'][0]['master_speed'] = $numerator / ($denominator ?: 1000);
                        }
                        if (isset($parsedXML['SPEED']['TIMECODE_RATE'])) {
                            @[$numerator, $denominator] = explode('/', (string) $parsedXML['SPEED']['TIMECODE_RATE']);
            ...
            case 'AVI ':
                ...
                if (isset($thisfile_riff[$RIFFsubtype]['movi']['offset'])) {
                    ...
                    if (isset($thisfile_riff['AVIX'])) {
                        $info['avdataend'] = $thisfile_riff['AVIX'][((is_countable($thisfile_riff['AVIX']) ? count($thisfile_riff['AVIX']) : 0) - 1)]['chunks']['movi']['offset'] + $thisfile_riff['AVIX'][((is_countable($thisfile_riff['AVIX']) ? count($thisfile_riff['AVIX']) : 0) - 1)]['chunks']['movi']['size'];
                    } else {
                ...
                if (isset($thisfile_riff['AVI ']['hdrl']['avih'][$streamindex]['data'])) {
                    ...
                    $thisfile_riff_video_current['frame_rate'] = round(1_000_000 / $thisfile_riff_raw_avih['dwMicroSecPerFrame'], 3);
...
$info['error'][] = 'WebP image parsing not supported in this version of getID3()';
        ...
        if (isset($thisfile_riff_raw['strh'][0]['dwLength']) && isset($thisfile_riff_raw['avih']['dwMicroSecPerFrame'])) {
            ...
            $info['playtime_seconds'] = $thisfile_riff_raw['strh'][0]['dwLength'] * ($thisfile_riff_raw['avih']['dwMicroSecPerFrame'] / 1_000_000);
        } elseif (isset($thisfile_riff_raw['avih']['dwTotalFrames']) && isset($thisfile_riff_raw['avih']['dwMicroSecPerFrame'])) {
            $info['playtime_seconds'] = $thisfile_riff_raw['avih']['dwTotalFrames'] * ($thisfile_riff_raw['avih']['dwMicroSecPerFrame'] / 1_000_000);
        }
    ...
    public function ParseRIFFAMV($startoffset, $maxoffset)
    {
        ...
        try {
            ...
            $info['video']['frame_rate']   = 1_000_000 / $RIFFchunk['amvh']['us_per_frame'];
            ...
            while ($this->ftell() < $maxoffset) {
                ...
                switch ($chunkname) {
                    case 'LIST':
                        ...
                        if (preg_match('#^(movi|rec )$#i', (string) $listname)) {
                                ...
                                if ($AudioChunkStreamType == 'wb') {
                                    ...
                                    if (preg_match('/^\xFF[\xE2-\xE7\xF2-\xF7\xFA-\xFF][\x00-\xEB]/s', $FirstFourBytes)) {
                                        ...
                                        if (getid3_mp3::MPEGaudioHeaderBytesValid($FirstFourBytes)) {
                                            ...
                                            $getid3_mp3 = new getid3_mp3($getid3_temp, self::class);
                        ...
                        switch ($chunkname) {
                            case 'data':
                                ...
                                if (preg_match('/^\xFF[\xE2-\xE7\xF2-\xF7\xFA-\xFF][\x00-\xEB]/s', substr($testData, 0, 4))) {
                                    ...
                                    unset($getid3_temp, $getid3_ac3);
                                } elseif (preg_match('/^('.implode('|', array_map('preg_quote', getid3_dts::$syncwords)).')/', (string) $testData)) {
    ...
    public function ParseRIFFdata(&$RIFFdata)
    {
        ...
        if ($RIFFdata) {
            ...
            $getid3_temp->info['audio']        = ($info['audio'] ?? array());
            $getid3_temp->info['video']        = ($info['video'] ?? array());
