...
class getid3_lib
{
    public static function PrintHexBytes($string, $hex=true, $spaces=true, $htmlencoding='UTF-8')
    {
        ...
        for ($i = 0; $i < strlen((string) $string); $i++) {
            if ($hex) {
                ...
                $returnstring .= ' '.(preg_match("#[\x20-\x7E]#", (string) $string[$i]) ? $string[$i] : '¤');
            }
            if ($spaces) {
                $returnstring .= ' ';
            }
    ...
    public static function Bin2Dec($binstring, $signed=false)
    {
        ...
        for ($i = 0; $i < strlen((string) $binstring); $i++) {
            $decvalue += ((int) substr($binstring, strlen((string) $binstring) - $i - 1, 1)) * 2 ** $i;
        }
    ...
    public static function DateMac2Unix($macdate)
    {
        ...
        return self::CastAsInt($macdate - 2_082_844_800);
    }


    public static function FixedPoint8_8($rawdata)
    {
        return self::BigEndian2Int(substr($rawdata, 0, 1)) + (float) (self::BigEndian2Int(substr($rawdata, 1, 1)) / 2 ** 8);
    }


    public static function FixedPoint16_16($rawdata)
    {
        return self::BigEndian2Int(substr($rawdata, 0, 2)) + (float) (self::BigEndian2Int(substr($rawdata, 2, 2)) / 2 ** 16);
    }
    ...
    public static function FixedPoint2_30($rawdata)
    {
        ...
        return self::Bin2Dec(substr($binarystring, 0, 2)) + (float) (self::Bin2Dec(substr($binarystring, 2, 30)) / 2 ** 30);
    }
    ...
    public static function CreateDeepArray($ArrayPath, $Separator, $Value)
    {
        ...
        if (($pos = strpos($ArrayPath, (string) $Separator)) !== false) {
    ...
    public static function XML2array($XMLstring)
    {
        if (function_exists('simplexml_load_string') && function_exists('libxml_disable_entity_loader')) {
            ...
            $XMLobject = simplexml_load_string($XMLstring, \SimpleXMLElement::class, LIBXML_NOENT);
    ...
    public static function CopyFileParts($filename_source, $filename_dest, $offset, $length)
    {
        if (!self::intValueSupported($offset + $length)) {
            throw new Exception('cannot copy file portion, it extends beyond the '.round(PHP_INT_MAX / 1_073_741_824).'GB limit');
        }
                    ...
                    foreach ($tagdata as $key => $value) {
                        if (!empty($value)) {
                            if (empty($ThisFileInfo['comments'][$tagname])) {
                                ...
                                foreach ($ThisFileInfo['comments'][$tagname] as $existingkey => $existingvalue) {
                                    $oldvaluelength = strlen(trim($existingvalue));
                                    if ((strlen((string) $existingvalue) > 10) && ($newvaluelength > $oldvaluelength) && (substr(trim($value), 0, strlen((string) $existingvalue)) == $existingvalue)) {
